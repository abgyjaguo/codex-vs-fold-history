...
async function maybeDownloadManual() {
  ...
  for (const dir of candidates) {
    try {
      ...
      const outPath = path.join(dir, "codex-vs-fold-history-AI-Operator-Manual.md");
...
async function main() {
  ...
  results.push({
    ...
    verifyIncludes: "CODEX_WORKFLOW_FOLD_HOST_V8",
  });
  results.push({
    file: webviewJs,
    ...(await patchFile(webviewJs, patchWebviewBundleJs)),
    verifyIncludes: "CODEX_WORKFLOW_FOLD_PATCH_V19",
  });
  if (zhCnJs) {
    results.push({
      file: zhCnJs,
